...
_FFMPEG_DIR = Path(__file__).parent / "ffmpeg_bin"

# Статические сборки ffmpeg для разных платформ
_FFMPEG_BUILDS = {
    ...
    ("linux", "armv7l"):  "https://johnvansickle.com/ffmpeg/releases/ffmpeg-release-armhf-static.tar.xz",
    ("windows", "amd64"): "https://www.gyan.dev/ffmpeg/builds/ffmpeg-release-essentials.zip",
    ("darwin", "x86_64"): None,  # macOS — используй brew install ffmpeg
    ("darwin", "arm64"):  None,
}

def _ensure_ffmpeg() -> str | None:
    """
    Проверяет наличие ffmpeg в системе или в локальной папке ffmpeg_bin/.
    Если не найден — скачивает статический бинарник автоматически.
    Возвращает путь к ffmpeg или None при неудаче.
    """
    ...
    try:
        ...
        if result.returncode == 0:
            log.info("✅ ffmpeg найден в системе")
            return "ffmpeg"
    except (FileNotFoundError, subprocess.TimeoutExpired):
        pass

    # 2. Проверяем локальный ffmpeg в ffmpeg_bin/
    local = _FFMPEG_DIR / ("ffmpeg.exe" if sys.platform == "win32" else "ffmpeg")
    if local.exists():
        log.info("✅ ffmpeg найден локально: %s", local)
        _add_ffmpeg_to_path()
        return str(local)

    # 3. Скачиваем
    log.info("⬇️ ffmpeg не найден, скачиваю статическую сборку...")
    _FFMPEG_DIR.mkdir(parents=True, exist_ok=True)

    system   = sys.platform  # "linux", "win32", "darwin"
    machine  = platform.machine().lower()  # "x86_64", "aarch64", "amd64", etc.

    # Нормализуем названия платформ
    if system == "win32":
        sys_key = "windows"
        machine = "amd64"
    elif system == "darwin":
    ...
    url = _FFMPEG_BUILDS.get((sys_key, machine))

    if url is None:
        log.warning(
            "⚠️ Автоустановка ffmpeg не поддерживается для %s/%s. "
            "Установите вручную: brew install ffmpeg (macOS) или apt install ffmpeg (Linux)",
            sys_key, machine
        )
        return None

    archive_path = _FFMPEG_DIR / ("ffmpeg_dl.zip" if url.endswith(".zip") else "ffmpeg_dl.tar.xz")

    try:
        log.info("⬇️ Загружаю: %s", url)
        urllib.request.urlretrieve(url, archive_path)
        log.info("📦 Распаковываю...")

        if url.endswith(".zip"):
            _extract_ffmpeg_zip(archive_path)
        else:
            _extract_ffmpeg_tar(archive_path)

        archive_path.unlink(missing_ok=True)

    except Exception as e:
        log.error("❌ Ошибка при скачивании ffmpeg: %s", e)
        return None

    if local.exists():
        _add_ffmpeg_to_path()
        log.info("✅ ffmpeg успешно установлен: %s", local)
        return str(local)
    else:
        log.error("❌ ffmpeg не найден после распаковки")
        return None


def _extract_ffmpeg_tar(archive: Path):
    """Распаковывает .tar.xz от johnvansickle.com и достаёт бинарники ffmpeg и ffprobe."""
    with tarfile.open(archive, "r:xz") as tf:
        for member in tf.getmembers():
            basename = Path(member.name).name
            if basename in ("ffmpeg", "ffprobe") and member.isfile():
                member.name = basename  # кладём прямо в _FFMPEG_DIR без вложенных папок
                tf.extract(member, _FFMPEG_DIR)
                extracted = _FFMPEG_DIR / basename
                extracted.chmod(extracted.stat().st_mode | stat.S_IEXEC | stat.S_IXGRP | stat.S_IXOTH)
                log.info("  ✅ извлечён: %s", extracted)


def _extract_ffmpeg_zip(archive: Path):
    """Распаковывает .zip от gyan.dev (Windows) и достаёт ffmpeg.exe."""
    with zipfile.ZipFile(archive, "r") as zf:
        for name in zf.namelist():
            basename = Path(name).name
            if basename in ("ffmpeg.exe", "ffprobe.exe"):
                data = zf.read(name)
                dest = _FFMPEG_DIR / basename
                dest.write_bytes(data)
                log.info("  ✅ извлечён: %s", dest)


def _add_ffmpeg_to_path():
    """Добавляет ffmpeg_bin/ в PATH и настраивает yt-dlp использовать локальный ffmpeg."""
    ffmpeg_dir_str = str(_FFMPEG_DIR.resolve())
...
def _base_opts(vid: str) -> dict:
    ...
    # Если ffmpeg лежит локально — явно указываем путь
    local_ffmpeg = _FFMPEG_DIR / ("ffmpeg.exe" if sys.platform == "win32" else "ffmpeg")
    if local_ffmpeg.exists():
        opts["ffmpeg_location"] = str(_FFMPEG_DIR)
    return opts
...
def main():
    ...
    # Автоустановка ffmpeg — проверяем/скачиваем при старте
    ffmpeg_path = _ensure_ffmpeg()
    if ffmpeg_path:
        log.info("✅ ffmpeg готов: %s", ffmpeg_path)
    else:
        log.error(
            "❌ ffmpeg недоступен! Конвертация в MP3 не будет работать.\n"
            "   Установите вручную:\n"
